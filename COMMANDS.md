...
解释：应用成功后自动再测一轮对比（本地把同一条 iperf3 命令再运行 1 次即可）：只要复测低于优化前（留 5% 容差）就自动恢复原参数并保留 BBR + FQ，绝不保留更慢的结果。测速端口只在防火墙原本未放行时新增规则，测速结束（含失败路径）删除；原本存在的规则不会被删除。菜单第 2 项按快照恢复运行态并删除调优文件。当前内核不支持 BBR 时不写入任何配置。
### 5.16 禁用 IPv6
