...
解释：应用成功后自动再测一轮对比：iperf3 复测低于优化前 50% 时提示是否恢复；TCPquality 只报告对比结果、不自动回滚。测速端口只在防火墙原本未放行时新增规则，测速结束（含失败路径）删除；原本存在的规则不会被删除。菜单第 2 项按快照恢复运行态并删除调优文件。当前内核不支持 BBR 时不写入任何配置。
### 5.16 禁用 IPv6
