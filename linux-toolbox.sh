	  ...
	  case $sub_choice in
		  1)
			  while true; do
				  ...
				  if [ "$kuaijiejian" != "d" ]; then
					  ln -sf /usr/local/bin/d /usr/local/bin/$kuaijiejian
				  fi
				  ln -sf /usr/local/bin/d /usr/bin/$kuaijiejian > /dev/null 2>&1
				  echo "快捷键已设置"
				  send_stats "脚本快捷键已设置"
				  break_end
				  linux_Settings
...
ssl_nginx_manager() {
	...
	ssl_install_acme() {
		if [ ! -f "$ACME" ]; then
			...
			if [ ! -f "$ACME" ]; then
				echo -e "${gl_hong}acme.sh 安装失败${gl_bai}"
				return 1
			fi
		fi
	...
	ssl_install_nginx() {
		...
		echo -e "${gl_lv}nginx 已安装并启动${gl_bai}"
	}
	...
	ssl_show_dns() {
		...
		if command -v dig >/dev/null 2>&1; then
			echo -e "${gl_huang}DNS A 记录: $(dig +short "$d" A | tr '\n' ' ')${gl_bai}"
			echo -e "${gl_huang}DNS AAAA记录: $(dig +short "$d" AAAA | tr '\n' ' ')${gl_bai}"
		fi
	}

	ssl_stop_port_80_services() {
		if systemctl is-active nginx >/dev/null 2>&1; then
			echo -e "${gl_huang}检测到 nginx 正在运行，正在停止...${gl_bai}"
		...
		if lsof -iTCP:80 -sTCP:LISTEN >/dev/null 2>&1; then
			echo -e "${gl_hong}80端口仍被占用${gl_bai}"
			return 1
		fi
		echo -e "${gl_lv}80端口已空闲${gl_bai}"
	}

	ssl_restart_nginx_if_needed() {
		if [ "$NGINX_WAS_RUNNING" = true ]; then
			systemctl start nginx
			echo -e "${gl_lv}nginx 已重新启动${gl_bai}"
		fi
	}

	ssl_setup_cron() {
		local CRON_LINE="0 3 * * * $ACME --cron --home $HOME/.acme.sh >/dev/null 2>&1"
		( crontab -l 2>/dev/null | grep -v "$ACME --cron" || true; echo "$CRON_LINE" ) | crontab -
		echo -e "${gl_lv}已配置自动续期（每天凌晨3点）${gl_bai}"
	}

	ssl_show_cert_list() {
		echo -e "${gl_lv}=========================================${gl_bai}"
		echo -e "${gl_lv}acme.sh 已注册的证书列表：${gl_bai}"
		"$ACME" --list 2>/dev/null || echo -e "${gl_huang}暂无已注册的证书${gl_bai}"
		echo ""
		echo -e "${gl_lv}/root/domain 下的证书文件夹：${gl_bai}"
		[ -d "/root/domain" ] && ls -la /root/domain/ 2>/dev/null || echo -e "${gl_huang}目录为空${gl_bai}"
		echo -e "${gl_lv}=========================================${gl_bai}"
	}
	...
	ssl_issue_cert() {
		local DOMAIN="$1"
		ssl_show_dns "$DOMAIN"
		local FOLDER=$(echo "$DOMAIN" | cut -d'.' -f1)
		local CERT_DIR="/root/domain/$FOLDER"
		mkdir -p "$CERT_DIR"
		if "$ACME" --list 2>/dev/null | grep -q "$DOMAIN"; then
			echo -e "${gl_huang}检测到已有证书，将强制重新申请...${gl_bai}"
			"$ACME" --remove -d "$DOMAIN" 2>/dev/null || true
			rm -rf "$HOME/.acme.sh/${DOMAIN}_ecc" "$HOME/.acme.sh/$DOMAIN" 2>/dev/null || true
		fi
		ssl_ensure_ufw_80
		ssl_stop_port_80_services || return 1
		...
		if [ -s "$CERT_DIR/fullchain.pem" ] && [ -s "$CERT_DIR/privkey.pem" ]; then
			echo -e "${gl_lv}证书申请成功！证书路径: $CERT_DIR${gl_bai}"
			ls -la "$CERT_DIR"
			ssl_setup_cron
			return 0
		else
			echo -e "${gl_hong}证书安装失败${gl_bai}"
			return 1
		fi
	}
	...
	ssl_config_nginx() {
		...
		local CERT_DIR="/root/domain/$FOLDER"
		if [ ! -s "$CERT_DIR/fullchain.pem" ] || [ ! -s "$CERT_DIR/privkey.pem" ]; then
			echo -e "${gl_hong}证书文件不存在: $CERT_DIR，请先申请证书${gl_bai}"
			return 1
		fi
		ssl_install_nginx
		local NGINX_CONF="/etc/nginx/sites-available/$NGINX_NAME"
...
EOF
		...
		if nginx -t; then
			...
			echo -e "${gl_lv}nginx 配置成功！配置文件: $NGINX_CONF，域名: $DOMAIN，代理端口: $PORT${gl_bai}"
			systemctl status nginx --no-pager || true
		else
			echo -e "${gl_hong}nginx 配置检测失败${gl_bai}"
			rm -f "$NGINX_CONF"
			return 1
		fi
	}
	...
	ssl_remove_cert() {
		ssl_show_cert_list
		read -e -p "请输入要移除的域名: " REMOVE_DOMAIN
		[ -z "$REMOVE_DOMAIN" ] && echo -e "${gl_hong}域名不能为空${gl_bai}" && return 1
		read -e -p "确认移除 $REMOVE_DOMAIN？[y/n]: " CONFIRM
		[ "$CONFIRM" != "y" ] && return 0
		"$ACME" --remove -d "$REMOVE_DOMAIN" 2>/dev/null || true
		local FOLDER=$(echo "$REMOVE_DOMAIN" | cut -d'.' -f1)
		[ -d "/root/domain/$FOLDER" ] && rm -rf "/root/domain/$FOLDER"
		echo -e "${gl_lv}证书移除完成！${gl_bai}"
	}

	ssl_remove_nginx_config() {
		local configs=(); local i=1
		echo -e "${gl_lv}当前 nginx 配置文件：${gl_bai}"
		for f in /etc/nginx/sites-available/*; do
			[ -f "$f" ] || continue
			local name=$(basename "$f")
			[ "$name" = "default" ] && continue
			configs+=("$name"); echo "$i) $name"; ((i++))
		done
		[ ${#configs[@]} -eq 0 ] && echo -e "${gl_huang}暂无自定义配置${gl_bai}" && return 0
		read -e -p "请输入要删除的配置编号: " NUM
		if ! [[ "$NUM" =~ ^[0-9]+$ ]] || [ "$NUM" -lt 1 ] || [ "$NUM" -gt ${#configs[@]} ]; then echo -e "${gl_hong}无效选择${gl_bai}"; return 1; fi
		local CONF_NAME="${configs[$((NUM-1))]}"
		read -e -p "确认删除 $CONF_NAME？[y/n]: " CONFIRM
		[ "$CONFIRM" != "y" ] && return 0
		rm -f "/etc/nginx/sites-available/$CONF_NAME" "/etc/nginx/sites-enabled/$CONF_NAME"
		nginx -t && systemctl reload nginx
		echo -e "${gl_lv}配置 $CONF_NAME 已删除${gl_bai}"
	}

	ssl_remove_nginx_and_cert() {
		local configs=(); local i=1
		echo -e "${gl_lv}当前 nginx 配置文件：${gl_bai}"
		for f in /etc/nginx/sites-available/*; do
			[ -f "$f" ] || continue
		...
		echo -e "${gl_huang}将删除：$CONF_FILE ${CERT_DIR:+和证书目录 $CERT_DIR} ${DOMAIN:+以及 acme 证书 $DOMAIN}${gl_bai}"
		...
		nginx -t && systemctl reload nginx
		echo -e "${gl_lv}nginx 配置和证书已删除${gl_bai}"
	}

	ssl_create_test_page() {
		read -e -p "请输入测试文件名称: " TEST_NAME
		[ -z "$TEST_NAME" ] && echo -e "${gl_hong}名称不能为空${gl_bai}" && return 1
		read -e -p "请输入测试端口号: " TEST_PORT
		[ -z "$TEST_PORT" ] && echo -e "${gl_hong}端口号不能为空${gl_bai}" && return 1
		ssl_install_nginx
		mkdir -p "/var/www/$TEST_NAME"
		cat > "/var/www/$TEST_NAME/index.html" << EOF
<!DOCTYPE html><html><head><title>$TEST_NAME 测试页面</title></head><body><h1>$TEST_NAME - 端口 $TEST_PORT 测试成功！</h1><p>测试名称: $TEST_NAME</p><p>监听端口: $TEST_PORT</p></body></html>
EOF
...
EOF
		...
		if nginx -t; then
			nginx -s reload
			echo -e "${gl_lv}测试页面创建成功！访问地址: http://127.0.0.1:$TEST_PORT${gl_bai}"
			curl "http://127.0.0.1:$TEST_PORT" 2>/dev/null || true
		else
			echo -e "${gl_hong}nginx 配置检测失败${gl_bai}"
			rm -f "/etc/nginx/sites-available/$TEST_NAME"; rm -rf "/var/www/$TEST_NAME"; return 1
		fi
	}

	ssl_remove_test_page() {
		local tests=(); local i=1
		echo -e "${gl_lv}当前测试页面：${gl_bai}"
		for d in /var/www/*/; do
			[ -d "$d" ] || continue
			local name=$(basename "$d")
			[ "$name" = "html" ] && continue
			[ -f "/etc/nginx/sites-available/$name" ] || continue
			tests+=("$name"); echo "$i) $name"; ((i++))
		done
		[ ${#tests[@]} -eq 0 ] && echo -e "${gl_huang}暂无测试页面${gl_bai}" && return 0
		read -e -p "请输入要删除的测试页面编号: " NUM
		if ! [[ "$NUM" =~ ^[0-9]+$ ]] || [ "$NUM" -lt 1 ] || [ "$NUM" -gt ${#tests[@]} ]; then echo -e "${gl_hong}无效选择${gl_bai}"; return 1; fi
		local TEST_NAME="${tests[$((NUM-1))]}"
		read -e -p "确认删除 $TEST_NAME？[y/n]: " CONFIRM
		[ "$CONFIRM" != "y" ] && return 0
		rm -f "/etc/nginx/sites-enabled/$TEST_NAME" "/etc/nginx/sites-available/$TEST_NAME"
		rm -rf "/var/www/$TEST_NAME"
		nginx -t && nginx -s reload
		echo -e "${gl_lv}测试页面 $TEST_NAME 已删除${gl_bai}"
	}
	...
	while true; do
		clear
		echo -e "${gl_lv}=========================================${gl_bai}"
		echo -e "${gl_lv}       SSL 证书 & Nginx 管理工具${gl_bai}"
		echo -e "${gl_lv}=========================================${gl_bai}"
		...
		echo "0) 返回主菜单"
		echo -e "${gl_lv}=========================================${gl_bai}"
		read -e -p "请选择操作 [0-9]: " ACTION
		case $ACTION in
			1)
				read -e -p "请输入域名: " DOMAIN; [ -z "$DOMAIN" ] && echo -e "${gl_hong}域名不能为空${gl_bai}" && break_end && continue
				read -e -p "请输入 nginx 配置文件名称: " NGINX_NAME; [ -z "$NGINX_NAME" ] && echo -e "${gl_hong}名称不能为空${gl_bai}" && break_end && continue
				read -e -p "请输入代理端口号: " PORT; [ -z "$PORT" ] && echo -e "${gl_hong}端口号不能为空${gl_bai}" && break_end && continue
				if ssl_issue_cert "$DOMAIN"; then ssl_restart_nginx_if_needed; ssl_config_nginx "$DOMAIN" "$NGINX_NAME" "$PORT"; else ssl_restart_nginx_if_needed; fi
				;;
			2) ssl_remove_nginx_and_cert ;;
			3) read -e -p "请输入域名: " DOMAIN; [ -n "$DOMAIN" ] && ssl_issue_cert "$DOMAIN"; ssl_restart_nginx_if_needed ;;
			4) ssl_remove_cert ;;
			5) ssl_show_cert_list ;;
			6)
				read -e -p "请输入域名: " DOMAIN; [ -z "$DOMAIN" ] && echo -e "${gl_hong}域名不能为空${gl_bai}" && break_end && continue
				read -e -p "请输入 nginx 配置文件名称: " NGINX_NAME; [ -z "$NGINX_NAME" ] && echo -e "${gl_hong}名称不能为空${gl_bai}" && break_end && continue
				read -e -p "请输入代理端口号: " PORT; [ -z "$PORT" ] && echo -e "${gl_hong}端口号不能为空${gl_bai}" && break_end && continue
				ssl_config_nginx "$DOMAIN" "$NGINX_NAME" "$PORT"
				;;
			7) ssl_remove_nginx_config ;;
			8) ssl_create_test_page ;;
			9) ssl_remove_test_page ;;
			0) break ;;
