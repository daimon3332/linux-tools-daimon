...
更新使用主菜单 `00`；不是 `d update`（后者升级系统软件包）。优先下载 GitHub 仓库版本，国内先走代理；仓库不可用才使用可能存在同步延迟的备用站点。更新会校验 Bash 语法并原子替换快捷命令，启动不会再采用当前目录中的同名旧脚本。交互输入结束时取消当前操作，不按默认选项继续执行。
...
| 8 | 动态 TCP 优化 | 运行 TCPquality 单线程测速，按实测带宽和 RTT 计算并应用参数；当前内核不支持 BBR 时停止操作，不写入无效配置 |
...
- `iperf3`：脚本监听临时端口并打印**一条**本地客户端命令 `iperf3 -c <IP> -p <端口> -R -t 20 -O 4 -i 1`，本地只需运行 1 次；脚本按服务端日志里预热 4 秒之后的每秒采样取中位数（同时打印均值/最低/最高），不再要求手动跑 3 次。RTT 优先取测试连接的 TCP 采样值，其次 ICMP，都没有时才要求手工输入。iperf3 服务端使用 `--forceflush` 输出，避免重定向到文件时缓冲导致读不到结果。
...
应用成功后自动再测一轮对比（本地再运行同一条命令 1 次）：只要复测低于优化前（留 5% 波动容差），就自动恢复原参数并保留 BBR + FQ，绝不保留更慢的结果；只有复测不低于优化前才保留动态参数。菜单第 2 项也可手动恢复，同样保留 BBR + FQ。测速临时端口只在防火墙原本未放行时新增规则，测速结束（包括失败路径）删除，原本存在的规则不会被删除。调优前保存一次运行态快照，菜单第 2 项据此恢复。当前内核不支持 BBR 时不写入任何配置，可选择主菜单 13 安装兼容内核。iperf3 与 TCPquality 结果都不保证所有线路都变快，实际效果见 [审计记录](AUDIT.md)。
