...
- `iperf3`：脚本监听临时端口并打印本地客户端命令，本地单线程下载测 3 次（波动超过 1.2 倍时提示补测到 5 次）取中位数；RTT 优先取测试连接的 TCP 采样值，其次 ICMP，都没有时才要求手工输入。iperf3 服务端使用 `--forceflush` 输出，避免重定向到文件时缓冲导致读不到结果。
...
应用成功后自动再测一轮对比：iperf3 复测低于优化前 50% 时提示是否恢复（跨境链路时段波动可达数倍，阈值过紧会误回滚）；TCPquality 走公共端点波动更大，只报告对比结果，不自动回滚。测速临时端口只在防火墙原本未放行时新增规则，测速结束（包括失败路径）删除，原本存在的规则不会被删除。调优前保存一次运行态快照，菜单第 2 项据此恢复。当前内核不支持 BBR 时不写入任何配置，可选择主菜单 13 安装兼容内核。iperf3 与 TCPquality 结果都不保证所有线路都变快，实际效果见 [审计记录](AUDIT.md)。
