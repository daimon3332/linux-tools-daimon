...
应用成功后自动再测一轮对比，复测低于优化前 80% 时提示是否恢复。测速临时端口只在防火墙原本未放行时新增规则，测速结束（包括失败路径）删除，原本存在的规则不会被删除。调优前保存一次运行态快照，菜单第 2 项据此恢复。当前内核不支持 BBR 时不写入任何配置，可选择主菜单 13 安装兼容内核。iperf3 与 TCPquality 结果都不保证所有线路都变快，实际效果见 [审计记录](AUDIT.md)。
